import { buildDocKey, getConfig, isDiskRefFileId, resolveTransferProfile, saveConfig, type EditorType } from "./config";
import {
...
import { parseDiskDocumentId } from "./transfer/disk-ref";
import {
...
class LadcraftR7App {
  ...
  private applyDiskRefContext(): string {
    const documentId = parseDiskDocumentId(window.Asc?.plugin?.info ?? {});
    if (documentId == null) {
      throw new Error("Откройте документ с Р7-Диска (в URL должен быть id документа).");
    }
    this.contextState = "synced";
    this.contextError = null;
    this.contextFileId = `r7-disk:${documentId}`;
    this.contextFileName =
      window.Asc?.plugin?.info?.title ??
      window.Asc?.plugin?.info?.documentTitle ??
      "document.docx";
    this.boundDocKey = null;
    return `Документ на диске (id=${documentId})`;
  }
  ...
  private async recoverStaleSession(
    ...
    if (this.currentTransferProfile() === "disk-ref") {
    ...
    const paint = (forcePaint = false): void => {
      ...
      renderChatView(
        this.root,
        {
          ...
          diskRef: this.currentTransferProfile() === "disk-ref",
        },
        {
          onBack: () => {
            void this.exitChatToShell(true);
  ...
  private async handleRefreshContext(): Promise<void> {
    ...
    if (this.currentTransferProfile() === "disk-ref") {
      ...
      try {
        ...
        this.chatReady = false;
        this.renderChatShell(`Ошибка: ${this.contextError}`);
      }
      return;
    }
  ...
  private async refreshContextState(): Promise<void> {
    if (isDiskRefFileId(this.contextFileId) || this.currentTransferProfile() === "disk-ref") {
      try {
        const status = this.applyDiskRefContext();
        void status;
      } catch (err) {
