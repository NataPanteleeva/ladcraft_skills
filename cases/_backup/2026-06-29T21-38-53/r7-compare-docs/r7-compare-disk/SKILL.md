...
description: Transport для r7-compare-docs — list/fetch шаблонов и хост-документа через Р7-Диск API (без VFS snapshot); авто-поиск папки templates.
version: 1.1.0
mcp_spec:
...
| Tool | Когда | Вход |
|------|-------|------|
| `r7_list_disk_templates` | START | `{ host_document_id }` из supplement (опц. `directory_id` для отладки) |
| `r7_fetch_disk_template` | COMPARE (A) | `{ template_name }` или `{ document_id }` |
| `r7_fetch_disk_document` | COMPARE (B) | `{ document_id }` из `r7-disk:{id}` |
