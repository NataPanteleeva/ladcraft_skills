...
async function handler(state, params) {
	const normalized = await normalizeInputAsync(state, params);
	if (!normalized.ok) {
 ...
 * @returns {Promise<{ ok: true, data: NormalizedInput } | { ok: false, error: string }>}
 */
async function normalizeInputAsync(state, params) {
	...
	const explicitFileName = pickString(
		raw.fileName,
		render?.fileName,
		renderLocalPath ? renderLocalPath.split('/').pop() : ''
	);
	const mimeType =
	...
	let localPath = pickString(raw.localPath, render?.localPath);
	let fileName = explicitFileName ? sanitizeFileName(explicitFileName) : '';
	const actions = normalizeActions(raw.actions);

	if (!content_base64 && !localPath && explicitFileName) {
		localPath = `/workspace/out/${sanitizeFileName(explicitFileName)}`;
		fileName = sanitizeFileName(explicitFileName);
	}

	if (!content_base64 && !localPath) {
		const latest = await findLatestDocxInOutDir(state);
		if (latest) {
			localPath = latest.path;
	...
	if (!content_base64 && !localPath) {
		return {
			ok: false,
			error:
				'Нужен fileName из r7_render_docx, render/localPath или готовый DOCX в /workspace/out/. ' +
				'Не вызывай r7_deliver_docx с пустым {}.'
		};
	}

	if (!fileName) {
		const tail = localPath.split('/').pop();
		fileName = sanitizeFileName(tail || 'сравнение.docx');
	}
 ...
 */
async function findLatestDocxInOutDir(state) {
	const vfs = getVfsAdapter(state);
	if (!vfs || typeof vfs.listDir !== 'function') {
		return null;
	}
	try {
		const raw = await vfs.listDir('/workspace/out');
		const entries = Array.isArray(raw) ? raw : [];
		/** @type {{ name: string, path: string, mtime: number }[]} */
		const files = [];
		for (let i = 0; i < entries.length; i += 1) {
			...
			if (!name || !name.toLowerCase().endsWith('.docx')) continue;
			const isDir = record.isDirectory === true || record.type === 'directory';
			if (isDir) continue;
			let mtime = 0;
			if (typeof record.mtime === 'number' && Number.isFinite(record.mtime)) {
				mtime = record.mtime;
			} else if (typeof record.modifiedAt === 'number' && Number.isFinite(record.modifiedAt)) {
				mtime = record.modifiedAt;
			}
			files.push({
				name,
