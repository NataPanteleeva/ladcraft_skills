...
const AGENT_ID = "ju4MekTiV4psav71nudMI";
...
const DEFAULT_SKILL_CATALOG_IDS = {
  ...
  "r7-docx-render-s27": "UHOuXEcDX0QYUuEXSuS8T"
};
...
function buildSaveCompareDiskPayload() {
  ...
  return {
    ...
    name: slug,
    description:
      "DOCX отчёта сравнения на Р7-Диск в папку CompareResults (content_base64 или markdown).",
    detailed_description: readSkillBody(skillDir),
    version: "1.2.0",
    category: "productivity",
    icon: "document",
        ...
        schemas: {
          input: {
            ...
            properties: {
              ...
              markdown: { type: "string", description: "Markdown-отчёт (fallback)." },
              fileName: { type: "string", description: "Имя DOCX." },
              folderName: { type: "string", description: "Имя папки (по умолчанию CompareResults)." },
              folder_id: {
                type: "integer",
                description: "Опционально. ID папки CompareResults из предыдущего сохранения."
              }
            }
          },
          output: {
...
function buildDocxRenderPayload() {
  ...
  return {
    ...
    name: slug,
    description:
      "Собирает .docx из CompareReport для s27 — следующий шаг r7_save_compare_report_to_disk.",
...
function main() {
  ...
  const installedIds = [
    ...
    binds[1].installed_application_id || getInstalledId(saveCompareDisk.appId),
    binds[2].installed_application_id || getInstalledId(docxRender.appId)
  ];
