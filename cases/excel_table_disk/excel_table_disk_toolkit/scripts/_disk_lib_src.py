...
REPORT_FOLDER_SCAN_MAX_ID = 300
DEFAULT_REPORT_FOLDER_NAME = "Таблицы для отчета"


def _folder_names_match(a: str, b: str) -> bool:
    return str(a or "").strip().lower() == str(b or "").strip().lower()


async def _fetch_directory_entry(base_url: str, auth_token: str, directory_id: int) -> dict | None:
    status, raw = _http_request(
        "GET",
        f"{base_url}/api/v1/DocumentDirectory/Get?id={directory_id}",
    ...
    try:
        ...
        return None
    entry = _pick_directory_entry(payload)
    return entry if isinstance(entry, dict) else None


async def discover_report_folder_directory_id(
    base_url: str,
    auth_token: str,
    folder_name: str = DEFAULT_REPORT_FOLDER_NAME,
    max_id: int = REPORT_FOLDER_SCAN_MAX_ID,
) -> int | None:
    target = str(folder_name or "").strip()
    if not target:
        return None
    limit = max_id if max_id > 0 else REPORT_FOLDER_SCAN_MAX_ID
    fallback: int | None = None
    for directory_id in range(2, limit + 1):
        entry = await _fetch_directory_entry(base_url, auth_token, directory_id)
        if not entry:
            continue
        name = pick_string(entry.get("Name"), entry.get("Title"))
        if not _folder_names_match(name, target):
            continue
        entry_id = resolve_positive_id(entry.get("Id")) or directory_id
        parent_id = resolve_positive_id(entry.get("ParentId"))
        if parent_id is not None and parent_id > 0:
            parent = await _fetch_directory_entry(base_url, auth_token, parent_id)
            parent_name = pick_string(parent.get("Name"), parent.get("Title")) if parent else ""
            if MY_DOCS_NAME_RE.search(parent_name):
                return entry_id
        if fallback is None:
            fallback = entry_id
    return fallback
...
def _download_not_found(downloaded: dict) -> bool:
    error = str(downloaded.get("error", ""))
    return "404" in error or "not found" in error.lower() or "Document not found" in error


async def resolve_csv_document_by_name(
    base_url: str,
    auth_token: str,
    csv_name: str,
    directory_id: int | None = None,
    report_folder_name: str = DEFAULT_REPORT_FOLDER_NAME,
) -> dict:
    dirs_to_try: list[int] = []
    if directory_id is not None:
        dirs_to_try.append(directory_id)
    folder_id = await discover_report_folder_directory_id(base_url, auth_token, report_folder_name)
    if folder_id is not None and folder_id not in dirs_to_try:
        dirs_to_try.append(folder_id)
    for dir_id in dirs_to_try:
        lookup = await get_document_id_by_name(base_url, auth_token, dir_id, csv_name)
        doc_id = lookup.get("documentId") if lookup.get("ok") else None
        if doc_id is not None:
            return {"ok": True, "document_id": doc_id, "directory_id": dir_id}
    return {"ok": False, "error": f"CSV «{csv_name}» не найден по имени в папке отчёта."}
...
MY_DOCS_NAME_RE = re.compile(r"мои\s*документ|my\s*documents", re.I)
...
def _pick_directory_entry(payload: Any) -> dict:
...
def looks_like_my_documents_root(entry: dict, name: str) -> bool:
    title = str(name or "").strip()
    parent = entry.get("Parent") if isinstance(entry.get("Parent"), dict) else None
    parent_id = parent.get("Id") if parent else entry.get("ParentId")
    if parent_id in (None, 0):
        return not re.match(
            r"^(общ|common|shared|корзин|избран|ладкрафт|recycle|favorites?|recent|file\s*depot)",
            title,
            re.I,
        )
    return bool(MY_DOCS_NAME_RE.search(title))


async def verify_personal_root_directory(base_url: str, auth_token: str, directory_id: int) -> int | None:
    if directory_id <= 0:
        return None
    status, raw = _http_request(
        "GET",
    ...
    entry = _pick_directory_entry(payload)
    name = pick_string(entry.get("Name"), entry.get("Title"))
    if MY_DOCS_NAME_RE.search(name) or looks_like_my_documents_root(entry, name):
        return directory_id
    return None


async def resolve_personal_root_directory_id(
    base_url: str, auth_token: str, skill_storage, user_env: dict
) -> int | None:
    if skill_storage:
        cached = resolve_positive_id(skill_storage.get("r7_disk_my_documents_directory_id"))
        if cached is not None:
            verified = await verify_personal_root_directory(base_url, auth_token, cached)
            if verified is not None:
                return verified
    status, raw = _http_request(
        "GET",
        f"{base_url}/api/v1/People/GetProfile",
        headers={"Authorization": auth_token},
    )
    if status < 400:
        try:
            ...
            if isinstance(data, dict):
                for key in (
                    ...
                    candidate = resolve_positive_id(data.get(key))
                    if candidate is None:
                        continue
                    verified = await verify_personal_root_directory(base_url, auth_token, candidate)
                    if verified is not None:
                        if skill_storage:
                            skill_storage.set("r7_disk_my_documents_directory_id", str(verified))
                        return verified
        except json.JSONDecodeError:
            pass
    for candidate_id in [1, 0, *range(2, 129)]:
        verified = await verify_personal_root_directory(base_url, auth_token, candidate_id)
        if verified is not None:
            if skill_storage:
                skill_storage.set("r7_disk_my_documents_directory_id", str(verified))
            return verified
    env_personal = resolve_positive_id(user_env.get("ANALYTICS_CSV_PERSONAL_DIRECTORY_ID"))
    if env_personal is not None:
        return await verify_personal_root_directory(base_url, auth_token, env_personal)
    return None
