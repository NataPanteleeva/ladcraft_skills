# Миграция навыка в формат `cursor_ladcraft`

Этот документ описывает перенос навыка из старого Ladcraft/runtime формата или стороннего JSON в текущий формат `cursor_ladcraft`.

Цель миграции: не просто разложить файлы по папкам, а привести каждый tool к **`async function handler(state, params)`** в `scripts/*.js` и согласованным meta — тому контракту, который реально понимают dev-server и publish (устаревший вид без `handler` в целевом виде не оставляем).
...
| Старое поле | Куда переносить в `cursor_ladcraft` |
|-------------|-------------------------------------|
...
- `environment` переносят в `*.meta.md`, но dev-server берёт его из `SKILL.md`.
- В старом коде предполагается широкий KV/VFS API без явного контракта в `handler`.

Если просто перенести код без приведения к **`async function handler(state, params)`**, получится навык, который выглядит знакомо, но расходится с контрактом `cursor_ladcraft`.
...
в `cursor_ladcraft` это должно быть объявлено так:
...
Не переносите этот контракт в `*.meta.md` как основной источник истины.
...
2. `environment` объявлен только в `SKILL.md`.
...
Если хотя бы один пункт не выполнен, навык ещё не мигрирован в формат `cursor_ladcraft`.
