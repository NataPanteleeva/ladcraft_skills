...
description: Выполняет shell-команду на сервере через paramiko SSH.
...
Требует runtime-адаптер `paramikoSsh`. Учётные данные — из `environment.user` при установке навыка.
