...
async function main() {
  ...
  const r7Local = path.join(__dirname, "..", "compare-r7", "fixtures", "r7-word_smoketest.json");
  ...
  const wait = await waitRun(token, base, agentId, sessionId, 180000);
  const hist = await api(token, base, "GET", "/v1/agent/session/" + sessionId + "/history?page=1&size=50");
  const assistant = (hist.data || hist).find((m) => m.role === "assistant");
  const tools = (assistant && assistant.tool_calls ? assistant.tool_calls : []).map((tc) => ({
    name: tc.name,
    command: tc.command || "",
  }));
  const cmds = tools.map((t) => t.command).join(" ");
  const ok =
    wait.done &&
    !tools.some((t) => t.name === "bash") &&
    (cmds.includes("startup_session") ||
      ((cmds.includes("resolve_r7_document") || cmds.includes("list_session_files")) &&
        cmds.includes("list_templates"))) &&
    !cmds.includes("startup_compare") &&
    !cmds.includes("doc-compare");
  console.log(
    JSON.stringify(
      {
        ok,
        ...
        tool_calls: tools,
        content_preview: assistant && assistant.content ? assistant.content.slice(0, 400) : "",
      },
      null,
      2
    )
