...
function isLikelyTemplatePicker(text: string, choices: string[]): boolean {
  ...
  // Require real file names in the RAW choices — never after canonicalTemplateName().
...
export function collectPresentedTemplateChoices(
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (ordered.length && isLikelyTemplatePicker(text, ordered)) {
      return ordered.map(canonicalTemplateName);
    }

    const widgetChoices = extractWidgetChoices(message, messages, i);
    const fromText = extractChoicesFromText(text);
    const rawMerged = [...ordered];
    for (const choice of [...widgetChoices, ...fromText]) {
      if (!rawMerged.some((x) => x.toLowerCase() === choice.toLowerCase())) {
        rawMerged.push(choice);
      }
    }

    // Critical: decide on RAW labels. Mapping .md first made every analytics
    // numbered list look like a compare-template picker.
    if (!isLikelyTemplatePicker(text, rawMerged)) continue;
...
export function resolveTemplateSelection(
  ...
  if (!choices.length) {
    // Bare `foo.md` only when it looks like a short template file name — not a
    // long analytics option that was wrongly suffixed.
    if (/^[^\s\\/]{1,80}\.(?:md|docx)$/i.test(t) && !/\*\*|—/.test(t)) {
