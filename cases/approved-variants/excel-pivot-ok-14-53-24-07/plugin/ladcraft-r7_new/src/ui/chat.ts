...
function patchChrome(
  ...
  if (sendBtn) {
    if (state.isSending) {
      ...
      sendBtn.innerHTML =
        '<span class="agent-spinner send-btn-spinner" aria-hidden="true"></span><span>Ждём…</span>';
    } else {
...
function buildActionIconButton(
  ...
  // Spreadsheet strip: unlock as soon as Файл:/tool path is known (no wait for isSending).
  btn.disabled = !state.chatReady || (state.isSending && !(isSpreadsheet && hasXlsx));
...
function renderMessage(m: ChatMessage, callbacks?: ChatViewCallbacks): HTMLElement {
  ...
  if (m.role === "assistant") {
    ...
    const body = el("div", "message-body");
    if (m.text.trim()) {
      // Never show spurious `….md` from false template-normalize on analytics picks.
      body.textContent = stripChoiceArtifacts(m.text);
    }
    node.appendChild(body);
  }
