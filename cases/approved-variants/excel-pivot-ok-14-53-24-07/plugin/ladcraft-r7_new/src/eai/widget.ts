...
function resolveWidgetHtml(raw: string): string | null {
  ...
  // Any real HTML/markup from the agent — show as-is (not only known compare slugs).
...
export function extractWidgetPayload(message: HistoryMessage): ChatWidgetPayload | null {
  const candidates = [
    message.widget_html?.trim(),
    message.widget_name?.trim(),
...
/** True when history says the agent asked via widget / waiting_user (may lack HTML yet). */
export function isAgentWidgetOrWait(message: HistoryMessage): boolean {
  return isWidgetMessage(message) || isWaitingForUserInput(message);
}
