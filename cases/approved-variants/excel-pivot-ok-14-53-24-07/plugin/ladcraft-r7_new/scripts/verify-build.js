...
const EXPECT_VERSION = "0.6.62-sheet-callback";
...
const requiredSnippets = ["documentCallbackUrl"];
for (const snippet of requiredSnippets) {
  if (!dist.includes(snippet)) {
    fail(`dist/app.js missing ${snippet}`);
  }
}
ok("dist/app.js required symbols");
for (const snippet of [
  ...
  if (!dist.includes(snippet)) console.log("WARN: optional missing:", snippet);
}
