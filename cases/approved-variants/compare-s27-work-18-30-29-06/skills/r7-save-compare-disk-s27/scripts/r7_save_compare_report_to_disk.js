...
async function handler(state, params) {
	...
	let rootId = loginResult.my_documents_directory_id;
	if (rootId == null) {
		const discovered = await discoverPersonalRootQuick(baseUrl, authToken);
		if (discovered) {
			rootId = discovered.id;
			if (skillStorage) {
				skillStorage.set('r7_disk_my_documents_directory_id', String(rootId));
			}
		}
	}
	if (rootId == null) {
...
async function diskLogin(baseUrl, login, password, skillStorage) {
	...
	if (myDocumentsDirectoryId == null) {
		const discovered = await discoverPersonalRootQuick(baseUrl, authToken);
		if (discovered) {
			myDocumentsDirectoryId = discovered.id;
		}
	}
	...
	if (skillStorage) {
		...
		if (myDocumentsDirectoryId != null) {
			skillStorage.set('r7_disk_my_documents_directory_id', String(myDocumentsDirectoryId));
		}
	}
...
async function discoverPersonalRootQuick(baseUrl, authToken) {
	for (let i = 0; i < QUICK_ROOT_PROBE_IDS.length; i += 1) {
		const candidateId = QUICK_ROOT_PROBE_IDS[i];
		...
		const entryId = typeof fetched.entry.Id === 'number' ? fetched.entry.Id : candidateId;
