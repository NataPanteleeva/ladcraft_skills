...
- Абзацы / списки / `#` — обычный текст. `**…**` — акценты.
- Не оборачивай весь черновик в `>` (рамка+фон в Word). `>` только для цитаты.
- Длинные «раздел | описание» — не GFM-таблица; `**Раздел.** текст` или заголовки.
