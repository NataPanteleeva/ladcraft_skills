...
function normalizeReport(params) {
	...
	let sections = normalizeSections(report.sections);
	let meta = report.meta && typeof report.meta === 'object' ? /** @type {Record<string, unknown>} */ (report.meta) : null;

	if (!meta && (report.template || report.document || report.divergencesCount != null)) {
		meta = {
			documentA: report.template ? { name: pickString(report.template) } : undefined,
			documentB: report.document ? { name: pickString(report.document) } : undefined,
			totalDiffs: typeof report.divergencesCount === 'number' ? report.divergencesCount : undefined
		};
	}

	if (sections.length === 0 && Array.isArray(report.tables)) {
		/** @type {ReportTable[]} */
		const rootTables = [];
		for (const t of report.tables) {
			const table = normalizeTable(t);
			if (table) rootTables.push(table);
		}
		if (rootTables.length > 0) {
			sections.push({ heading: 'Расхождения', level: 2, tables: rootTables, quotes: [] });
		}
	}

	if (sections.length === 0 && Array.isArray(report.sections)) {
		const flatTable = normalizeFlatDiffSections(report.sections);
		if (flatTable) {
			sections.push({ heading: 'Расхождения', level: 2, tables: [flatTable], quotes: [] });
		}
	}
...
/**
 * Legacy flat diff items: { param, template, document, severity }.
 * @param {unknown} sectionsRaw
 * @returns {ReportTable | null}
 */
function normalizeFlatDiffSections(sectionsRaw) {
	...
	for (const item of sectionsRaw) {
		if (!item || typeof item !== 'object') continue;
		const s = /** @type {Record<string, unknown>} */ (item);
		if (Array.isArray(s.tables)) continue;
		const param = pickString(s.param, s.label, s.id != null ? String(s.id) : '');
		const template = pickString(s.template, s.templateText, s.etalon);
		const document = pickString(s.document, s.docText, s.doc);
		const severity = pickString(s.severity, s.type, s.mark);
		if (!param && !template && !document) continue;
		rows.push([param, template, document, severity]);
	}
