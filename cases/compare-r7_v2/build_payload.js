...
function buildToolSkill(skillDir) {
  ...
  const compareCommonPath = path.join(scriptsDir, "_compare_common.js");
  const commonCode = fs.existsSync(commonPath)
    ? fs.readFileSync(commonPath, "utf8").trim() + "\n"
    : "";
  const compareCommonCode =
    skillName === "r7-compare-toolkit" && fs.existsSync(compareCommonPath)
      ? fs.readFileSync(compareCommonPath, "utf8").trim() + "\n"
      : "";
  const tools = toolNames.map((name) => {
    ...
    const needsDocxCommon = /docx|render/.test(name);
    return {
      name,
      description:
      ...
      function: handler + "\n" + (needsDocxCommon ? commonCode : "") + compareCommonCode + libCode,
    };
  });
...
function main() {
  ...
  const skillDirs = skillArg
    ...
    : [
        path.join(ROOT, "skills", "r7-document-compare"),
  ...
  for (const skillDir of skillDirs) {
    ...
    const skillPath = path.join(skillDir, "SKILL.md");
    const { fm } = parseSkillMd(skillPath);
    const mcpTools = (fm.mcp_spec && fm.mcp_spec.tools) || [];
    const toolNames = mcpTools.map((t) => (typeof t === "string" ? t : t.name)).filter(Boolean);
    const hasScripts = fs.existsSync(path.join(skillDir, "scripts"));
    const payload =
      toolNames.length > 0 && hasScripts ? buildToolSkill(skillDir) : buildInstructionSkill(skillDir);
