...
function renderActionBlock(block: ActionBlock, handlers: ActionHandlers): HTMLElement {
  ...
  if (block.kind === "insert") {
    ...
    const baseName = block.baseName ?? "отчёт";
    row.appendChild(
      buildButton("Скачать .md", () =>
        runAction(handlers, () => downloadMarkdown(handlers.client, block.payload, baseName)),
      ),
    );
    row.appendChild(
      buildButton("Скачать для Word (.html)", () =>
        runAction(handlers, () => downloadAsWordHtml(handlers.client, block.payload, baseName)),
      ),
    );
  }
