...
import { extractTasksFromReply, getMessageFullText } from "./executor";
...
/** True when object matches CompareReport shape. */
export function isCompareReport(value: unknown): value is CompareReport {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const obj = value as Record<string, unknown>;
  if (obj.schema === "doc-compare/v1") return true;
  if (!Array.isArray(obj.sections) || !obj.sections.length) return false;
  const first = obj.sections[0];
  if (!first || typeof first !== "object") return false;
  return Array.isArray((first as CompareSection).tables);
}
...
function collectCompareReportCandidates(message: HistoryMessage): unknown[] {
  ...
  pushParsed(getMessageFullText(message));
