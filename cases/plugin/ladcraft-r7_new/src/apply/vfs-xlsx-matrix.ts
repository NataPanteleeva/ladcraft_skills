...
export async function loadAgentXlsxMatrix(
  ...
  sessionId: string,
  vfsPath: string,
  options: { excludePath?: string | null; sheet?: string } = {},
): Promise<XlsxMatrixResult> {
  const sheetKey = options.sheet ? `::${options.sheet}` : "";
  const key = cacheKey(sessionId, vfsPath) + sheetKey;
  ...
  const blob = await downloadVfsFile(client, resolved.fileId, "original");
  ...
  matrixCache.set(cacheKey(sessionId, resolved.vfsPath) + sheetKey, matrix);
  return matrix;
}
