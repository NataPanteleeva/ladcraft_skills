...
import { getStreamingVisibleText } from "../src/apply/content-extract";
...
// stream: hide r7.task and partial task JSON while SSE is active
{
  const partial =
    '```r7.task\n[{"type":"search_replace","data":{"search":"x","replace":"y","matchCase":false}}';
  const partialVisible = getStreamingVisibleText(partial);
  assert(
    partialVisible === "Агент выполняет запрос…",
    "partial r7.task fence hidden in stream",
  );

  const withPrefix =
    "Сейчас внесу правку.\n" +
    '```r7.task\n[{"type":"search_replace","data":{"search":"x","replace":"y"}}]';
  const prefixVisible = getStreamingVisibleText(withPrefix);
  assert(prefixVisible === "Сейчас внесу правку.", "prefix kept, r7.task tail hidden");

  const inline =
    '[{"type":"search_replace","data":{"search":"a","replace":"b","matchCase":false}}]';
  const inlineVisible = getStreamingVisibleText(inline);
  assert(
    inlineVisible === "Агент выполняет запрос…",
    "inline task JSON hidden in stream",
  );
  ok("stream sanitize: hide r7.task and task JSON");
}
...
// suggested actions layer 1: text hints without widget
{
  const reportText =
    "## Результаты сравнения\n\n" +
    "| Пункт | Шаблон | Документ | Комментарий | Метка |\n|---|---|---|---|---|\n" +
    ...
    "**Расхождений: 7**\n\n" +
    "Чтобы вставить отчёт в документ, напишите: **вставить**\n" +
    "Чтобы скачать отчёт (в формате md), напишите: **скачать**\n" +
    "Чтобы сохранить отчёт на Р7-Диск в формате docx, напишите: **сохранить на диск**";
  const items: HistoryMessage[] = [{ id: "report-hints", role: "assistant", content: reportText }];
  const messages = historyToChatMessages(items, { editorType: "word" });
  const report = messages.find((m) => m.id === "report-hints");
  assert(Boolean(report?.suggestedActions?.length), "suggested actions from text hints");
  assert(
    report?.suggestedActions?.some((a) => a.send === "вставить"),
    "insert suggested action",
  );
  assert(
    report?.suggestedActions?.some((a) => a.send === "сохранить на диск"),
    "save-to-disk suggested action",
  );
  assert(
    report?.suggestedActions?.some((a) => a.send === "скачать md"),
    "download md suggested action",
  );
  assert(
    !report?.suggestedActions?.some((a) => a.send === "скачать docx"),
    "no duplicate docx suggested action",
  );
  assert(!report?.actionPlan?.blocks.length, "no layer-2 buttons before intent");
  assert(!report?.text.includes("Чтобы вставить"), "hint lines stripped from bubble");
  ok("suggested actions: text hints without widget");
}

// partial r7.actions + text hints: merge all three actions
{
  const reportText =
    "## Результаты сравнения\n\n**Расхождений: 7**\n\n" +
    "```r7.actions\n" +
    '[{"label":"вставить отчёт в документ","send":"вставить","primary":true},' +
    '{"label":"сохранить отчёт на Р7-Диск в формате docx","send":"сохранить на диск"}]\n' +
    "```\n\n" +
    "Чтобы скачать отчёт (в формате md), напишите: **скачать**";
  const items: HistoryMessage[] = [{ id: "report-partial", role: "assistant", content: reportText }];
  const messages = historyToChatMessages(items, { editorType: "word" });
  const report = messages.find((m) => m.id === "report-partial");
  assert(report?.suggestedActions?.length === 3, "merge r7.actions + text hints");
  assert(
    report?.suggestedActions?.some((a) => a.send === "скачать md"),
    "download md from text hint when r7.actions partial",
  );
  ok("suggested actions: merge partial r7.actions with text hints");
}

// suggested actions: r7.actions fenced block
{
  const reportText =
    "## Результаты\n\nРасхождений: 2\n\n" +
    "```r7.actions\n" +
    '[{"label":"Вставить","send":"вставить","primary":true},{"label":"Скачать","send":"скачать"}]\n' +
    "```";
  const items: HistoryMessage[] = [{ id: "report-actions", role: "assistant", content: reportText }];
  const messages = historyToChatMessages(items, { editorType: "word" });
  const report = messages.find((m) => m.id === "report-actions");
  assert(report?.suggestedActions?.length === 2, "r7.actions parsed");
  assert(!report?.text.includes("r7.actions"), "r7.actions hidden from bubble");
  ok("suggested actions: r7.actions block");
}

// template picker must not get suggested action buttons
{
  const items: HistoryMessage[] = [
    {
      id: "picker",
      role: "assistant",
      content:
        "Доступные шаблоны:\n\n| № | Шаблон |\n| 1 | `a.md` |\n\nКакой шаблон использовать? Напишите номер.",
    },
  ];
  const messages = historyToChatMessages(items, { editorType: "word" });
  const picker = messages.find((m) => m.id === "picker");
  assert(!picker?.suggestedActions?.length, "no suggested actions on template picker");
  ok("suggested actions: template picker excluded");
}

// widget present suppresses suggested actions duplicate
{
  const reportText = "## Результаты сравнения\n\nРасхождений: 1\n\nЧтобы вставить, напишите: вставить";
  const items: HistoryMessage[] = [
    {
      id: "report-widget",
      role: "assistant",
  ...
  const messages = historyToChatMessages(items, { editorType: "word" });
  const report = messages.find((m) => m.id === "report-widget");
  assert(Boolean(report?.widget?.html), "widget attached");
  assert(!report?.suggestedActions?.length, "suggested actions suppressed when widget present");
  ok("suggested actions: suppressed when widget iframe");
}
