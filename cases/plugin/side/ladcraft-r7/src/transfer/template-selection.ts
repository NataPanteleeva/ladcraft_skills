...
function stemOf(mdName: string): string {
...
export function extractOrderedTemplateNames(text: string): string[] {
  ...
  const add = (raw: string): void => {
    const md = raw.match(MD_NAME_RE)?.[1]?.trim();
    if (!md) return;
...
function isLikelyTemplatePicker(text: string, choices: string[]): boolean {
  ...
  if (/^\s*\d+[\.\):\-]\s*[^\n]+\.md/im.test(text)) return true;
  return choices.some((c) => /\.md$/i.test(c));
}
...
export function collectPresentedTemplateChoices(
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    const widgetChoices = extractWidgetChoices(message, messages, i)
      .map(ensureMdExtension)
      .filter((c) => /\.md$/i.test(c) && !/^\d+\.md$/i.test(c));
...
export function resolveTemplateSelection(
  ...
  if (!choices.length) {
    if (/\.md$/i.test(t)) {
      return { matched: true, canonicalMd: ensureMdExtension(t) };
    }
    return { matched: false };
  }

  const canonicalList = choices.map(ensureMdExtension);
