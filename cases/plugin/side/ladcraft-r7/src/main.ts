...
import { captureDiskDocumentIdFromEnvironment, parseDiskDocumentId } from "./transfer/disk-ref";
import {
    ...
    try {
      ...
      if (usesVfsSnapshot(transferProfile)) {
        ...
        try {
          ...
          status =
            "Чат открыт без id документа — нажмите «Обновить id документа» перед отправкой";
        }
      } else {
        status = "Готово";
      }
    ...
    const paint = (forcePaint = false): void => {
      ...
      renderChatView(
        this.root,
        {
          ...
          diskRef: usesDiskRef(this.currentTransferProfile()),
        },
        {
          onBack: () => {
            void this.exitChatToShell(true);
  ...
  private async handleSend(text: string): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        if (usesDiskRef(this.currentTransferProfile())) {
          throw new Error(
            `Контекст диска недоступен: ${this.contextError}. Нажмите «Обновить id документа».`,
          );
        }
        throw new Error(msg);
      }
