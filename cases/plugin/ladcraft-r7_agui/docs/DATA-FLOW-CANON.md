...
|--------------|---------------|---------------|
| tool → `/session/foo.xlsx` | Ищет path в tool result / строке `Файл:` | **Краткая справка** (факты из tool) + **обязательно** `Файл: /session/….xlsx` |
| — | Кнопки: **XLSX / Лист / Вставить / Заменить / CSV** | Можно коротко про кнопки **только** вместе с `Файл:`; справку на лист не писать |
...
| Фильтр / «вынеси по городу» | (`inspect` если колонка неясна) → `filter_export` | справка + `Файл:` | кнопки |
| KPI / «сколько / сумма» | `filter_export` и/или `top_n_summary` / `build_pivot_table` | справка + `Файл:` | кнопки |
| Сводная | `inspect` → `build_pivot_table` | справка + `Файл:` | кнопки |
