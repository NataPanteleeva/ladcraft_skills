...
**После apply с action bar / intent paste:** только `chatStatus` в chrome («Вставлено в документ» / …). **Не** добавлять assistant-пузырь `local-apply-*` — иначе `resolveActionTarget` берёт ack без insertable и панель «Действия» пропадает. Кнопки остаются на **последнем ответе модели** до нового хода агента; `resolveActionTarget` пропускает любые `local-*` id. Повторный клик: `allowRepeat: true`.

**Вставка текста:** `paste_text` → MD→HTML (`contentToPasteHtml`) → `PasteHtml` (`**`, `#`, списки, таблицы). Источник — `applyText` / proposal / Черновик, не DOM пузыря.
