...
| короткая таблица `\| … \|` + `\|---\|` | `<table class="md-table">` | оформленная таблица |
| длинная 2-кол. таблица (prose) | `.md-kv-stack` (ключ над значением) | блоки без узкой сетки |
| `# Заголовок` | `<h1>`…`<h6>` | заголовок |
| обычная строка | `<p>…</p>` | абзац |

**Правило для агента:** весь черновик не оборачивать в `>`; `>` только для настоящей цитаты / callout. Длинные «Раздел | описание» — заголовки/списки, не GFM-таблица. Короткие структуры (опечатки `№ | Было | Стало`) — таблица ок.

### Смена оформления по фразе (локально, без LLM)

| Фраза | Действие |
|-------|----------|
| «убери рамку / фон / оформление» | снять `>` у черновика или выделенного текста → paste/replace |
| «оформи как цитату» | обернуть строки в `>` |
| «сделай жирным» / «убери жирный» | добавить/снять `**` |
| «без таблицы / списком» | GFM-таблицы → `**ключ.** значение` |

Код: `src/apply/word-format-intent.ts`. Есть выделение → `replace_selection`; иначе вставка черновика у курсора.
...
| `src/apply/word-format-intent.ts` | локальная смена рамки/жирного/таблицы |
| `src/markdown/html.ts` | `markdownToHtml`, prose-table → kv-stack, `contentToPasteHtml` |
