...
export interface PluginFeatures {
  /** Live SSE token streaming during assistant turns (legacy GET /sse). */
  sseStreaming: boolean;
  /** AG-UI POST /v2/agent/run streaming (preferred in ladcraft-r7_agui). */
...
function readLocalOverride(): Partial<PluginFeatures> | null {
  try {
    ...
    if (typeof parsed.agUiStreaming === "boolean") out.agUiStreaming = parsed.agUiStreaming;
...
export function getPluginFeatures(): PluginFeatures {
  ...
  return {
    ...
    agUiStreaming: override?.agUiStreaming ?? preset.agUiStreaming,
    sseStreaming:
      override?.agUiStreaming === true
        ? false
        : legacySse ?? override?.sseStreaming ?? preset.sseStreaming,
...
export function savePluginFeatures(partial: Partial<PluginFeatures>): PluginFeatures {
  ...
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      agUiStreaming: next.agUiStreaming,
