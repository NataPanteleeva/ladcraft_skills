...
export function historyToChatMessages(
  ...
  // Orphan-retry can POST the same user phrase twice; keep one bubble in UI.
  ...
  for (const m of messages) {
    ...
    if (
      ...
      prev.role === "user" &&
      m.role === "user" &&
      normalizeUserBubbleText(prev.text) === normalizeUserBubbleText(m.text)
    ) {
      continue;
    }
    deduped.push(m);
...
function normalizeUserBubbleText(text: string): string {
