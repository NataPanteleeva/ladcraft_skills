...
const FORMAT_OPEN_SHEET_RE =
  /(?:оформи(?:те)?|форматируй(?:те)?|сделай(?:те)?\s+(?:жирн|шапк)|жирн(?:ый|ой|ую)?\s+шапк|заливка|шрифт|подчеркн|выдели(?:те)?\s+шапк|header\s+bold|выдели(?:те)?|закрась(?:те)?|закраши|подсвет|раздвинь|подгони\s+ширин|autofit|сам(?:ых|ые|ую|ая).*(?:низк|высок))/i;
