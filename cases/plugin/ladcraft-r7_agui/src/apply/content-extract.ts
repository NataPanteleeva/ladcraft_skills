import { sanitizeAssistantChatText } from "./display-sanitize";
...
export const STREAMING_TABLE_PLACEHOLDER = "Агент формирует таблицу…";
export const STREAMING_WORKING_PLACEHOLDER = "Агент выполняет запрос…";

/** Earliest index of partial r7.task fence or inline task JSON still streaming in. */
function findStreamingLeakStart(text: string): number {
  const markers = [
    /```r7\.task/i,
    /\[\s*\{[\s\S]*?"type"\s*:\s*"/,
    /\{\s*"type"\s*:\s*"(?:paste|paste_text|cell_paste|add_comment|search_replace|remove_selection|replace_selection|deliver_inline|deliver_file|share_link|open_file)"/,
  ];
  let earliest = -1;
...
/** Hide incomplete task/tool payloads before terminal history sanitize runs. */
function stripStreamingLeaks(text: string): string {
  const leakStart = findStreamingLeakStart(text);
  if (leakStart < 0) return text;
  return text.slice(0, leakStart).trimEnd();
}

/** Index of first markdown table line (`| col |`), or -1. */
export function findMarkdownTableStart(text: string): number {
  const lines = text.split(/\r?\n/);
  let offset = 0;
  for (let i = 0; i < lines.length; i++) {
    if (/^\s*\|/.test(lines[i])) return offset;
...
export function isSubstantiveResult(text: string): boolean {
  ...
  // Excel analytics: short «Фильтр… / Файл: /session/….xlsx» must end the wait.
  if (/(?:^|\n)\s*Файл:\s*\/session\/[^\s`"']+\.xlsx\b/im.test(body)) return true;
