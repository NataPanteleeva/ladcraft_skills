...
export interface R7SheetReplaceFromXlsxTask {
  type: "sheet_replace_from_xlsx";
  data: {
    path: string;
    sheet?: string;
    mode?: "used" | "selection" | "range" | string;
    range?: string | null;
    clearTarget?: boolean;
  };
}

export interface R7CellFormatTask {
  type: "cell_format";
  data: {
    target?: string;
    range?: string | null;
...
const TASK_TYPE_PATTERN =
  "paste|paste_text|cell_paste|add_comment|search_replace|remove_selection|replace_selection|sheet_replace_from_xlsx|cell_format|deliver_inline|deliver_file|share_link|open_file";
...
function isValidTask(task: R7Task): boolean {
  switch (task.type) {
    ...
    case "sheet_replace_from_xlsx":
      return Boolean(task.data && typeof task.data === "object" && typeof task.data.path === "string");
    case "cell_format":
      return Boolean(
        task.data &&
          typeof task.data === "object" &&
          ((task.data.format && typeof task.data.format === "object") ||
            (Array.isArray(task.data.rules) && task.data.rules.length > 0) ||
            task.data.autoFitColumns === true),
      );
    case "deliver_inline":
      return (
