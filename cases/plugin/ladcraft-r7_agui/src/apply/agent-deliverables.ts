...
export interface AgentDeliverable {
  ...
  fileName: string;
  mime: string;
  /** AG-UI / projection file_id — prefer for VFS download when present. */
  fileId?: string;
}
...
/**
 * All result .xlsx from the latest assistant turn (file refs → chat «Файл:» → tools).
 * Used when the agent produced several tables in one reply.
 */
export function listLatestAssistantDeliverables(
  messages: { role: string; text?: string; applyText?: string; fileReferences?: Array<{
    file_id: string;
    path?: string;
    display_name?: string;
    mime_type?: string;
  ...
  const byPath = new Map<string, AgentDeliverable>();
  const seen = new Set<string>();
  const exclude = options.excludePath
    ? normalizeDeliverablePath(options.excludePath)
    : null;

  const pushDeliverable = (d: AgentDeliverable | null | undefined) => {
    if (!d) return;
    const p = normalizeDeliverablePath(d.vfsPath);
    if (!p || seen.has(p)) {
      if (p && d.fileId && byPath.has(p) && !byPath.get(p)!.fileId) {
        byPath.get(p)!.fileId = d.fileId;
      }
      return;
    }
    ...
    seen.add(p);
    ordered.push(p);
    byPath.set(p, { ...d, vfsPath: p });
  };

  const pushPath = (path: string | null | undefined) => {
    if (!path) return;
    pushDeliverable(deliverableFromPath(path));
  };

  // 1) AG-UI / projection file_references on the latest assistant message
  for (let i = messages.length - 1; i >= 0; i--) {
    const m = messages[i];
    if (m.role !== "assistant") continue;
    ...
    if (Array.isArray(refs) && refs.length) {
      for (const ref of refs) {
        ...
        if (!fileId) continue;
        let vfsPath = String(ref.path || "").replace(/^~/, "").trim();
        ...
        if (!isXlsx) continue;
        pushDeliverable({ vfsPath, fileName, mime, fileId });
      }
      break;
    }
    break;
  ...
  // 2) Legacy history tool_calls (optional; AG-UI may leave history empty)
  if (history?.length) {
    for (let i = history.length - 1; i >= 0; i--) {
      const m = history[i];
      ...
      const content =
        ...
        "";
      for (const p of extractSessionXlsxPaths(content)) pushPath(p);
      ...
      for (const tc of calls) {
        ...
        const status = String(tc.status || "").toLowerCase();
        if (status && status !== "completed" && tc.success === false) continue;
        pushPath(targetPathFromToolResult(parseToolResultObject(tc.result)));
      }
      break;
    }
  }

  // 3) Chat text «Файл: /session/….xlsx»
  if (!ordered.length) {
    for (let i = messages.length - 1; i >= 0; i--) {
      const m = messages[i];
      if (m.role !== "assistant") continue;
      const raw = (m.applyText || m.text || "").trim();
      if (!raw) continue;
      for (const p of extractSessionXlsxPaths(raw)) pushPath(p);
      break;
    }
  }
  ...
  return ordered.map((p) => byPath.get(p) || deliverableFromPath(p));
}
...
/**
 * Analytics turn is actionable: file refs / Файл: line / (legacy) tool userReply.
 */
...
/** Ready from chat messages (AG-UI): fileReferences or Файл: in text. */
export function hasReadyAnalyticsFromChat(
  messages: {
    role: string;
    text?: string;
    applyText?: string;
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    const m = messages[i];
    if (m.role !== "assistant") continue;
    if (Array.isArray(m.fileReferences) && m.fileReferences.length) return true;
    const raw = (m.applyText || m.text || "").trim();
    if (hasSessionResultFileLine(raw)) return true;
    break;
  }
  return false;
}
