...
const TOOL_NAME_TO_TYPE: Record<string, R7TaskType> = {
  ...
  sheet_replace: "sheet_replace_from_xlsx",
  ...
  "sheet-replace": "sheet_replace_from_xlsx",
...
function toolCallToTask(call: ToolCallRecord): R7Task | null {
  ...
  // Keep type+data before normalize strips them (sheet_replace / cell_format).
  ...
  const parsed =
    obj && obj.data !== undefined ? obj.data : normalizePayload(raw);
  ...
  switch (type) {
    ...
    case "sheet_replace_from_xlsx": {
      ...
      const path =
        ...
        (typeof d.sourcePath === "string" && d.sourcePath) ||
        "";
      if (!path) return null;
      const modeRaw = typeof d.mode === "string" ? d.mode.toLowerCase() : "used";
      const mode =
        modeRaw === "selection" || modeRaw === "range" || modeRaw === "used"
          ? modeRaw
          : "used";
      return {
        type,
        data: {
          path,
          sheet:
            typeof d.sheet === "string"
              ? d.sheet
              : typeof d.sourceSheet === "string"
                ? d.sourceSheet
                : undefined,
          mode,
          range: typeof d.range === "string" ? d.range : null,
          clearTarget: d.clearTarget !== false,
        },
      };
    }
    case "cell_format": {
      ...
      const d = data as Record<string, unknown>;
      const format =
        d.format && typeof d.format === "object" && !Array.isArray(d.format)
          ? (d.format as Record<string, unknown>)
          : {};
      const rules = Array.isArray(d.rules)
        ...
        : null;
      const autoFitColumns = d.autoFitColumns === true;
      if (!Object.keys(format).length && !(rules && rules.length) && !autoFitColumns) {
        return null;
      }
      const cells = Array.isArray(d.cells)
        ? d.cells.map((x) => String(x || "").trim()).filter(Boolean)
        : null;
      return {
        type,
        data: {
          target: typeof d.target === "string" ? d.target : "used",
          range: typeof d.range === "string" ? d.range : null,
          cells,
          format,
          rules,
          applyScope: typeof d.applyScope === "string" ? d.applyScope : "row",
          autoFitColumns,
        },
      };
    }
