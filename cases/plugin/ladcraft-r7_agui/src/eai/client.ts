...
export class EaiClient {
  ...
  async request<T>(
    ...
    try {
      ...
      throw new Error(humanizeNetworkError(err, path));
    }
  ...
  /**
   * Authenticated fetch that returns the raw Response (for SSE / AG-UI streams).
   * Caller must read `response.body`; does not parse JSON.
   */
  async streamRequest(
    path: string,
    options: {
    ...
    const { method = "POST", body, auth = true, headers = {}, signal } = options;
    if (auth) await this.ensureFreshToken();
  ...
  async fetchBlob(path: string): Promise<Blob> {
    ...
    let res: Response;
    try {
      res = await fetch(`${this.getBaseUrl()}${path}`, { method: "GET", headers });
    } catch (err) {
      throw new Error(humanizeNetworkError(err, path));
    }
    if (!res.ok) {
...
/** Map browser TypeError "Failed to fetch" to actionable Russian text. */
export function humanizeNetworkError(err: unknown, path?: string): string {
  const raw = err instanceof Error ? err.message : String(err);
  const lower = raw.toLowerCase();
  const where = path?.includes("vfs") ? " (VFS)" : path?.includes("auth") ? " (авторизация)" : "";
  if (
    ...
    return `Нет связи с api.ladcraft.ru${where}. Проверьте интернет / VPN и войдите снова.`;
  }
  if (lower.includes("aborted") || lower.includes("timeout")) {
    return `Таймаут запроса к API${where}. Повторите «Синхр. документ» или откройте новый чат.`;
  }
