...
3. **Чат** — text-only SSE (`content_delta`) + финальный `history` sync; документ синхронизируется через `prepareOutbound`.
4. **Назад** — сброс сессии Ladcraft; привязка VFS документа в localStorage сохраняется.

### Streaming режима (block 2)

- Основной SSE: `GET /v1/agent/sse/{session_id}`.
- Candidate endpoint `GET /v1/agent-api/sessions/{session_id}/events` на текущем prod возвращал `403`; плагин переключается на `/v1/agent/sse/{session_id}`.
