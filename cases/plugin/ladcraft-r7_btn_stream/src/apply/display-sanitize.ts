...
/** Remove r7.task / r7.actions blocks, tool JSON, compare JSON, and base64 blobs from assistant chat display. */
export function sanitizeAssistantChatText(text: string): string {
  let out = stripAgentServiceMarkup(stripActionsMarkup(stripTaskMarkup(text)));
